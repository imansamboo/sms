<?php
   ...
   class Iman_API_Exception extends Exception
   {
   }
   class Iman_API
   {
       ...
       public function processRawReply (Iman_Request $r) {
           if ($this->debug) {
               echo $r->getRaw() . "\n";
           }
           $msg = $r->getRaw();
           $str = $this->_send($msg);
           if (!$str) {
               throw new Iman_API_Exception("Bad reply", 4004);
           }
           if ($this->debug) {
               echo $str . "\n";
           }
       ...
       public function process (Iman_Request $r) {
           if ($this->debug) {
               echo $r->getRaw() . "\n";
           }
       ...
       {
           $ret = @htmlentities($str, null, Iman_API::$encoding);
           ...
           if (!empty($str) && is_string($str) && !self::checkCreateXml($str)) {
               ...
               $str = htmlentities($str, null, Iman_API::$encoding);
           }
           return $str;
       }
       ...
       {
           return new Iman_Request ($xmlStr);
       }
       static function createReply ($xmlStr = null)
       {
           return new Iman_Reply ($xmlStr);
       }
       ...
       protected static function parseArray ($node)
       {
           ...
           foreach ($node->childNodes as $child) {
               $name = self::decode($child->nodeName);
               if ('item' !== $name) {
                   throw new Iman_API_Exception('Wrong message format', 4006);
               }
   ...
   class Iman_Request
   {
        ...
        * @return void
        *
        * @uses Iman_Request::__construct()
        */
       protected function _retrieveDataFromDom ($dom)
       {
           $arr = Iman_API::convertXmlToPhpObj($dom->documentElement);
       ...
       public function _getRequest ()
       {
           ...
           if (isset($this->misc)) {
               ...
               Iman_API::convertPhpObjToDom($this->misc, $miscElement, $dom);
           }
           ...
           Iman_API::convertPhpObjToDom($this->args, $cmdNode, $dom);
   ...
   class Iman_Reply
   {
       ...
       protected function _parseReply ($str = '')
       {
           ...
           if ((!is_array($arr) && trim($arr) == '') ||
               $arr['reply']['code'] == 4005)
           {
               throw new Iman_API_Exception("API is temporarily unavailable due to maintenance", 4005);
           }
       ...
       public function _getReply ()
       {
           ...
           $descNode->appendChild(
               $dom->createTextNode(Iman_API::encode($this->faultString))
           );
           $dataNode = $replyNode->appendChild($dom->createElement('data'));
           Iman_API::convertPhpObjToDom($this->value, $dataNode, $dom);
           if (0 < count($this->warnings)) {
               $warningsNode = $replyNode->appendChild($dom->createElement('warnings'));
               Iman_API::convertPhpObjToDom($this->warnings, $warningsNode, $dom);
           }
           $this->dom = $dom;
           foreach ($this->filters as $f) {
               $f->filter($this);
